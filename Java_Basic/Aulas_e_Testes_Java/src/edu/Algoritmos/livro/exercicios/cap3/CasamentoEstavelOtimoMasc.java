...
public class CasamentoEstavelOtimoMasc {
	int n; int tet = 0; int[] man; int[] wom; int[] rank;
	int[][] wmr; int[][] mwr; int[][] rmw; int[][] rwm;
	boolean[] single; int[] x; int[] y;
	String mlrCasais; int mp=0;
	
	public CasamentoEstavelOtimoMasc(int n) {
		this.n = n; this.man = new int[n]; this.wom = new int[n]; this.rank = new int[n];
		this.wmr = new int[man.length][rank.length]; this.mwr = new int[wom.length][rank.length];
		this.rmw = new int[rank.length][man.length]; this.rwm = new int[rank.length][wom.length];
		this.x = new int[man.length]; this.y = new int[wom.length]; single = new boolean[wom.length];
	}
	
	private void tente(int m, int r, int rm) {
		int w = wmr[m][r];
		
		if (single[w] & stable(m, w, r)) {
			x[m] = w; y[w] = m; single[w] = false;
			if(m < n-1) {tente(m+1, 1, rm + rmw[m][x[m]]);}
			else if(rm + rmw[m][x[m]] < mp|| mp == 0){print();}
			single[w] = true;
		}
	...
	public void teste() {
		...
		definir(prefM, prefW);
		String casais  = " casais |";
		System.out.println(casais.repeat(n-1) + " rm | rw");
		tente(1, 1, 0);
		System.out.println(mlrCasais);
	}
